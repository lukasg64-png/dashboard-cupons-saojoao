/**
 * server.js — Dashboard Cupons Unificado (v2 — SQLite + Memória)
 * 
 * Foco 100% em cupons VTEX com dados enriquecidos a nível de item.
 * Unifica ambas as diretorias em uma visão única.
 * 
 * NOVA ARQUITETURA:
 *  - Memória: apenas pedidos do dia atual (rápido)
 *  - SQLite: histórico consolidado (persistente)
 *  - node-cron: consolidamento diário automático
 *  - Backfill em background no startup
 * 
...
const HISTORY_MONTHS = parseInt(process.env.HISTORY_MONTHS || '1', 10);
const HISTORY_DAYS = HISTORY_MONTHS * 30;
...
/**
 * Calcula range de datas com base no dateMode.
 */
function getDateRange(dateMode, customStart, customEnd) {
  ...
  switch (dateMode) {
    ...
    case 'mes_anterior': {
      ...
      const month = brt.getUTCMonth(); // 0-indexed, current month
...
/**
 * Enriquece pedidos VTEX raw (do cache/VTEX) com dados do cadastro.
 * Retorna lista de pedidos formatados para o frontend/SQLite.
 * 
 * Aceita tanto o todayCache quanto um cache arbitrário (para backfill).
 */
function enrichOrders(rawCache) {
  const catMap = vtexSync.getCategoryMap();
  const list = [];

  Object.values(rawCache).forEach(order => {
...
/**
 * Busca dados de cupons combinando memória (hoje) + SQLite (histórico).
 * Esta é a função principal que responde às rotas da API.
 */
function getCouponData(dateMode, customStart, customEnd, filters = {}) {
  const today = getBrtDateStr(0);
  const { startDate, endDate } = getDateRange(dateMode, customStart, customEnd);
  
  let results = [];

  // 1. Dados do dia atual (memória)
  if (endDate >= today) {
    const todayData = enrichOrders(vtexSync.getTodayCache());
    // Filtrar apenas as datas dentro do range
    const todayFiltered = todayData.filter(o => o.date >= startDate && o.date <= endDate);
    results.push(...todayFiltered);
  }

  // 2. Dados históricos (SQLite) — apenas datas antes de hoje
  const historyEndDate = endDate < today ? endDate : getBrtDateStr(1);
  if (startDate < today) {
    const historyData = historyDB.queryByDateRange(startDate, historyEndDate, filters);
    results.push(...historyData);
  }

  // 3. Aplicar filtros que não foram passados ao SQLite (para dados de memória)
  if (filters.diretoria && filters.diretoria !== 'all') {
    results = results.filter(d => d.diretoria === filters.diretoria);
  }
  if (filters.distrital && filters.distrital !== 'all') {
    results = results.filter(d => d.distrital === filters.distrital);
  }
  if (filters.coordenador && filters.coordenador !== 'all') {
    results = results.filter(d => d.coordenador === filters.coordenador);
  }
  if (filters.store && filters.store !== 'all') {
    results = results.filter(d => d.store === filters.store);
  }
  if (filters.coupon && filters.coupon !== 'all') {
    results = results.filter(d => d.coupon === filters.coupon.toUpperCase().trim());
  }

  // De-duplicate (em caso de overlap memória/SQLite)
  const seen = new Set();
...
app.get('/api/coupons', (req, res) => {
  try {
    ...
    const data = getCouponData(dateMode, dateStart, dateEnd, {
      diretoria,
      distrital,
      coordenador,
      store: filial,
    ...
    res.json({
      status: 'ok',
      sync: syncState,
      totalOrders: syncState.todayCacheSize,
...
/**
 * GET /api/coupons/summary — Resumo hierárquico
 */
...
app.get('/api/filters', (req, res) => {
  try {
    const { dateMode, dateStart, dateEnd } = req.query;
    const data = getCouponData(dateMode || '15d', dateStart, dateEnd);
...
app.get('/api/health', (req, res) => {
  ...
  res.json({
    ...
    sync: syncState,
    todayOrders: syncState.todayCacheSize,
    history: dbStats,
...
/**
 * GET /api/history/status — Status detalhado do histórico SQLite
 */
app.get('/api/history/status', (req, res) => {
  try {
    const stats = historyDB.getStats();
    const consolidatedDates = historyDB.getConsolidatedDates();
    const lastBackfill = historyDB.getMeta('lastBackfill');
    const backfillDays = historyDB.getMeta('backfillDays');
    
    // Encontrar gaps (datas que deveriam existir mas não existem)
    const gaps = [];
    if (consolidatedDates.length > 0) {
      const start = new Date(consolidatedDates[0]);
      const end = new Date(consolidatedDates[consolidatedDates.length - 1]);
      const dateSet = new Set(consolidatedDates);
    ...
    res.json({
      ...
      gaps,
      lastBackfill,
      backfillDays: backfillDays ? parseInt(backfillDays) : null,
      syncState: vtexSync.getSyncState(),
    });
  } catch (err) {
    console.error('[/api/history/status]', err.message);
    res.status(500).json({ status: 'error', error: err.message });
  }
});
...
app.post('/api/vtex-sync', async (req, res) => {
  ...
  vtexSync.syncTodayOnly().catch(err =>
    console.error('[Manual Sync] Falhou:', err.message)
  );
});

/**
 * POST /api/backfill — Inicia backfill manual
 */
app.post('/api/backfill', async (req, res) => {
  const days = parseInt(req.query.days || HISTORY_DAYS, 10);
  res.json({ status: 'started', days });
  
  vtexSync.backfillDays(days, enrichOrders).catch(err =>
    console.error('[Manual Backfill] Falhou:', err.message)
  );
});

/**
 * POST /api/consolidate — Consolida um dia específico
 */
app.post('/api/consolidate', async (req, res) => {
  const dateStr = req.query.date || getBrtDateStr(1);
  res.json({ status: 'started', date: dateStr });
  
  vtexSync.consolidateDay(dateStr, enrichOrders).catch(err =>
    console.error('[Manual Consolidate] Falhou:', err.message)
  );
});
...
app.post('/api/admin/seed', (req, res) => {
  try {
    ...
    // Enriquecer e consolidar no SQLite
    const enriched = enrichOrders(orders);
    const today = getBrtDateStr(0);
    const historyOrders = enriched.filter(o => o.date < today);
    if (historyOrders.length > 0) {
      historyDB.insertOrders(historyOrders);
      console.log(`[Seed] ${historyOrders.length} pedidos históricos salvos no SQLite.`);
    }
    
    res.json({ status: 'ok', count, message: `Seed: ${count} pedidos processados (${historyOrders.length} históricos).` });
  } catch (err) {
...
app.listen(PORT, () => {
  console.log(`\n🚀 Dashboard Cupons Unificado v2 — http://localhost:${PORT}`);
  console.log(`📊 Cadastro: ${Object.keys(filiaisCadastro).length} filiais (C+L)`);
  console.log(`📦 Histórico: ${HISTORY_DAYS} dias configurados\n`);

  // 1. Sync dia atual após 5s
  setTimeout(() => {
    vtexSync.syncTodayOnly().catch(err => console.error('[Startup Sync]', err.message));
  }, 5000);

  // 2. Backfill do histórico em background (após 15s para não competir com sync)
  setTimeout(() => {
    const dbStats = historyDB.getStats();
    if (dbStats.daysConsolidated < HISTORY_DAYS * 0.8) {
      console.log(`[Startup] Iniciando backfill (${dbStats.daysConsolidated}/${HISTORY_DAYS} dias no banco)...`);
      vtexSync.backfillDays(HISTORY_DAYS, enrichOrders).catch(err => 
        console.error('[Startup Backfill]', err.message)
      );
    } else {
      console.log(`[Startup] Histórico OK (${dbStats.daysConsolidated} dias). Sem backfill necessário.`);
    }
  }, 15000);
  ...
  cron.schedule('5 3 * * *', async () => {
    console.log(`\n⏰ [Cron 00:05 BRT] Iniciando consolidação diária...`);
    try {
      const yesterday = getBrtDateStr(1);
      const count = await vtexSync.consolidateDay(yesterday, enrichOrders);
      console.log(`[Cron] ✅ Consolidados ${count} pedidos de ${yesterday}.`);
    } catch (err) {
